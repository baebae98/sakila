...
public class FilmListService {
	private FilmListDao filmListDao;	// FilmListDao 객체 선언
	
	public ArrayList<Film> getFilmList(int limit1, int limit2) {
		ArrayList<Film> returnList = null;
		
		filmListDao = new FilmListDao();	// 메소드를 호출하기 위해 객체 생성
		...
		try {
			...
			// 트랜잭션 실행
			returnList = filmListDao.selectFilmList(conn, limit1, limit2);
	...
	public int countFilmList() {
		System.out.println("Debug: [SERVICE] countFilmList 실행");
		int count = 0;
